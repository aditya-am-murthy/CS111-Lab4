...
struct thread_cache {
    ...
    uint32_t last_bucket; // Last bucket accessed (for flush optimization)
};
...
static _Thread_local struct thread_cache thread_cache = { .count = 0, .last_bucket = UINT32_MAX };
...
// Flush the thread-local cache to the hash table for a specific bucket
static void flush_cache(struct hash_table_v2 *hash_table, uint32_t bucket)
{
    ...
    for (size_t i = 0; i < thread_cache.count; ++i) {
        if (bernstein_hash(thread_cache.entries[i].key) % HASH_TABLE_CAPACITY == bucket) {
    ...
    for (size_t i = 0; i < thread_cache.count; ++i) {
        if (thread_cache.entries[i].key != NULL) {
            free(thread_cache.entries[i].key); // Free keys not added to this bucket
        }
    }
    thread_cache.count = 0;
    thread_cache.last_bucket = UINT32_MAX;
}

bool hash_table_v2_contains(struct hash_table_v2 *hash_table, const char *key)
{
    struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
    uint32_t bucket = bernstein_hash(key) % HASH_TABLE_CAPACITY;

    // Flush cache if it contains entries for this bucket
    if (thread_cache.count > 0 && thread_cache.last_bucket == bucket) {
        flush_cache(hash_table, bucket);
    }
...
void hash_table_v2_add_entry(struct hash_table_v2 *hash_table, const char *key, uint32_t value)
{
    ...
    // If cache is full or bucket changes, flush the cache
    if (thread_cache.count == CACHE_SIZE || (thread_cache.count > 0 && thread_cache.last_bucket != bucket)) {
        flush_cache(hash_table, thread_cache.last_bucket);
    }
    ...
    thread_cache.count++;
    thread_cache.last_bucket = bucket;

    // Flush immediately if cache is full
